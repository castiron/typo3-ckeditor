...
//			partIds = [ 'intUrlOptions', 'extUrlOptions', 'emailOptions', 'fileOptions' ],
	...
	var parseLink = function( editor, element )
	{
		...
		else if ( href && urlMatch )		// urlRegex matches empty strings, so need to check for href as well.
		{
			...
			retval.linkType = 'intUrl'; // sets the default link type for new links.
				...
				[
					{
						...
						items: 
						[
							['TYPO3 Page or Content Element', 'intUrl'],
							['External URL', 'extUrl'],
							['Email', 'email'],
							['File', 'file']
						],
						...
						[
							{
								...
								label: 'Select a Page',
								title: 'Select a Page',
								html : '<div id="ckeditorPageTree" style="background-color: #FFF; margin-top: 10px; height: 400px; border: 1px solid #A0A0A0; width: 300px;"></div><div style="clear: both">',
								setup: function(data)
								{
									// TODO: Fix dumb variable scope issue here.
									if(!this.treeApp) {
										this.treeApp = new TYPO3.Components.PageTree.CkApp();
										this.treeApp.render('ckeditorPageTree');
										selectedPid = data.t3pid;
										if(selectedPid) {
											this.treeApp.select(selectedPid);
										}
									}
								},
								commit : function( data )
								{
									if ( !data.t3pid ) data.t3pid = {};
									data.t3pid = this.treeApp.getSelectedPid();
								}
							}	
						]
					}					
