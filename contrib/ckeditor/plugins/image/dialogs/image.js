									...
									[
										{
											...
											onChange : function()
											{
												...
												// check to see if it's a DAM asset
												mediaRegex = /^media:([0-9]+)$/;
												mediaMatch = newUrl.match( mediaRegex );

   												if(mediaMatch != null && mediaMatch[1]) {
   												var conn = new Ext.data.Connection();
   												conn.request({
   													url: '/typo3/mod.php?M=web_txckeditorM1',
   													method: 'POST',
   													scope: this,
   													params: {'action':'getDamRecord','damRecUid':mediaMatch[1]},
   													success: function(response, opts) {
   														responseObj = Ext.decode(response.responseText);
   														this.setValue('/' + responseObj.pathString);
   													}
   												});
   											}
